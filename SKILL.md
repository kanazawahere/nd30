...
1. **`scripts/generate_docx.py` (MẶC ĐỊNH, nhất là khi chạy trong sandbox không có filesystem
   riêng, vd Gemini Spark):** tự viết 1 file JSON mô tả nội dung — schema đầy đủ nằm trong docstring
   đầu file, **ví dụ mẫu sẵn ở `examples/input-sample.json`** (đọc file này trước khi tự đoán tên
   field, khỏi mất công đoán sai). Chạy 1 lệnh duy nhất (sinh file + validate luôn):
   ```bash
