...
# =====================================================================
# examples/*.json qua generate_docx.py — mỗi loại VB đều phải generate + validate sạch (0 FAIL)
# =====================================================================

@pytest.mark.parametrize("name", ["input-sample.json", "cong_van.json", "quyet_dinh.json",
                                  "bien_ban.json", "thong_bao.json", "giay_moi.json",
                                  "nghi_quyet_hdnd.json", "nghi_quyet_hdnd_kem_quy_dinh.json",
                                  "quyet_dinh_ubnd_qppl.json"])
def test_examples_generate_va_validate_sach(name, tmp_path):
    import json as _json
    import generate_docx as gd  # noqa
    spec_path = ROOT / "examples" / name
    assert spec_path.is_file(), f"thiếu ví dụ {name}"
    spec = gd.load_spec(spec_path)
    doc = gd.generate(spec)
    out = tmp_path / (name.replace(".json", ".docx"))
    doc.save(str(out))
    profile = "minutes-administrative" if spec.get("profile") == "minutes-administrative" else "administrative"
    results = run_checks(out, profile, allow_placeholder=True)
    fails = [r for r in results if r[0] == FAIL]
    assert not fails, f"{name}: có {len(fails)} lỗi nặng: {fails}"
