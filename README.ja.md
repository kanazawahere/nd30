...
- **法規範文書**：人民評議会の決議（Mẫu 17+18、`references/the-thuc-qppl-nq-hdnd.md` 参照）と
  省人民委員会の決定（Mẫu 19+20、`references/the-thuc-qppl-qd-ubnd.md` 参照）のMVPに対応 —
  法律・法令・国会決議、国家主席の命令・決定、政府の政令、通達は未対応。
- **党文書の書式**（指導文書 36-HD/VPTW号）は未対応。
- 電子署名・公印には非対応 — それぞれの機関の文書管理プロセスに委ねます。

## よくある質問（FAQ）

**このリポジトリは完成していますか？コード/プロンプト/テンプレートを追加する必要がありますか？**
完成しています。`SKILL.md` はそれ自体が完全なシステムプロンプト（3つの絶対ルール＋5フェーズの
ワークフロー）であり、`scripts/build_docx.py` + `generate_docx.py` はすでに `.docx` 生成エンジン、
`templates/*.docx` はMarkdownの骨組みではなく実際のファイルです。「Xファイルを追加すべき」と
提案する前に、**まずリポジトリ内のファイルを開いて読んでください**。[`llms.txt`](./llms.txt)
冒頭の「対応済み — 再提案不要」の表も確認してください。

**GitHubのトップページしか読めない（ファイルツリーを取得できない）AI/エージェントです。どう使えばいいですか？**
これは実測済みの本当の制限です（`SKILL.md` の「2 CHẾ ĐỘ CHẠY」セクション参照）：GitHubの
トップページにはこのREADMEしか表示されず、`SKILL.md`/`scripts/` の内容は自動的には表示されません。
[`llms.txt`](./llms.txt) から始めて `SKILL.md` へと、各rawファイル
（`https://raw.githubusercontent.com/kanazawahere/nd30/main/<パス>`）を**自分から取得**する
必要があります。リンクを自動取得できない環境の場合は、ユーザーが `SKILL.md` の内容を直接
チャット欄に貼り付けてください。

**なぜMCPサーバーを使わないのですか？**
意図的な設計です — このスキルは純粋なPythonスクリプトであり、コード実行サンドボックスを持つ
あらゆるAIチャット（Gemini、Claude、ChatGPTなど）にリンクや内容を貼り付けるだけで動作し、
追加のインフラをインストール・運用する必要がありません。

**自分が読んでいるのが最新版かキャッシュされた古い版か、どう確認すればいいですか？**
AIに「nd30のSKILL_VERSIONはいくつですか？」と聞いてください — 返ってきた番号が
[`SKILL.md`](./SKILL.md) 内の `SKILL_VERSION` と一致しているはずです。一致しなければキャッシュ
された版を読んでいます（`SKILL.md` 内のGitHub raw CDNキャッシュに関する警告を参照）。

**なぜ余白・フォントサイズが固定値ではなく「20-25mm」のような範囲で書かれているのですか？**
政令30/2020/NĐ-CP号の付録Iが単一の固定値ではなく範囲を規定しているためです — 法令原文から
そのまま引用したものであり、勝手に単一の数値を作ったわけではありません。
