...
3 cách truyền input, chọn 1 (không cần tạo file tạm nếu sandbox không tiện):
    python3 scripts/generate_docx.py input.json -o output.docx        # từ file
    echo '{"header": {...}, ...}' | python3 scripts/generate_docx.py - -o output.docx   # stdin
    python3 scripts/generate_docx.py --json-string '{"header": {...}}' -o output.docx   # chuỗi trực tiếp
